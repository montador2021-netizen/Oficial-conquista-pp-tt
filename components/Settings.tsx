      ...
      <div className="space-y-4">
            ...
            {['product', 'assistance', 'waterproofing'].map((key) => (
              <div key={key} className="flex items-center justify-between">
                <label className="text-[9px] font-black text-gray-500 uppercase flex items-center gap-2">
                  ...
                  />
                  Meta de {key === 'product' ? 'Produtos' : key === 'assistance' ? 'Assistência' : 'Impermeabilização'} (R$)
                </label>
                <input 
                  type="number" 
                  value={tempTargets[key as 'product'|'assistance'|'waterproofing']}
                  onChange={(e) => setTempTargets({ ...tempTargets, [key]: Number(e.target.value) })}
                  className="w-1/3 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-2 text-gray-800 font-bold focus:border-purple-500 outline-none transition-colors"
                />
              </div>
            ))}
            <div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 space-y-6 shadow-sm">
          <h3 className="text-[10px] font-black text-rose-600 uppercase tracking-widest flex items-center gap-2">
            <Target size={14} /> Premiação Semanal
          </h3>
          <div className="space-y-4">
              <label className="flex items-center gap-2 text-[9px] font-black text-gray-500 uppercase">
                  <input type="checkbox" checked={tempTargets.premiacaoExtra?.ativo ?? false} onChange={(e) => setTempTargets({...tempTargets, premiacaoExtra: {...tempTargets.premiacaoExtra, ativo: e.target.checked}})}/>
                  Ativar Premiação Semanal
              </label>
              <div className="grid grid-cols-2 gap-2">
                <div>
                   <label className="text-[8px] font-bold text-gray-400 uppercase block mb-1">Início</label>
                   <input type="date" value={tempTargets.premiacaoExtra?.dataInicio ?? ''} onChange={(e) => setTempTargets({...tempTargets, premiacaoExtra: {...tempTargets.premiacaoExtra, dataInicio: e.target.value}})} className="w-full p-3 border rounded-xl text-xs"/>
                </div>
                <div>
                   <label className="text-[8px] font-bold text-gray-400 uppercase block mb-1">Fim</label>
                   <input type="date" value={tempTargets.premiacaoExtra?.dataFim ?? ''} onChange={(e) => setTempTargets({...tempTargets, premiacaoExtra: {...tempTargets.premiacaoExtra, dataFim: e.target.value}})} className="w-full p-3 border rounded-xl text-xs"/>
                </div>
              </div>
              <input type="number" placeholder="Meta Valor" value={tempTargets.premiacaoExtra?.metaValor ?? 0} onChange={(e) => setTempTargets({...tempTargets, premiacaoExtra: {...tempTargets.premiacaoExtra, metaValor: Number(e.target.value)}})} className="w-full p-3 border rounded-xl"/>
              <input type="number" placeholder="Valor Prêmio" value={tempTargets.premiacaoExtra?.valorPremio ?? 0} onChange={(e) => setTempTargets({...tempTargets, premiacaoExtra: {...tempTargets.premiacaoExtra, valorPremio: Number(e.target.value)}})} className="w-full p-3 border rounded-xl"/>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className={`transition-opacity duration-300 ${tempTargets.bonusPorPedido?.ativo ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
              ...
              <input 
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-gray-800 font-bold text-sm outline-none focus:border-blue-500"
              />
            </div>
          </div>
        </div>
